...
# 1) Popular o Firestore com os dados de demonstração
#    (12 países, 18 vagas, 14 tecnologias, 7 trilhas)
python3 seed_firestore.py             # envia/atualiza (pode rodar várias vezes, não duplica)
python3 seed_firestore.py --limpar    # apaga as coleções e recomeça do zero
python3 seed_firestore.py --dry-run   # só lista o que seria enviado, sem conectar
...
- `seed_firestore.py` — script para popular o Firestore com os dados de demonstração.
...
## Como conferir no Firebase Console

Depois de rodar o seed, abra **Build → Firestore Database → Dados**. Você verá 4 coleções:

| Coleção | Docs | ID de exemplo |
|---|---|---|
| `paises` | 12 | `portugal` |
| `vagas` | 18 | `canada_maplestack_devops_engineer` |
| `radar_tecnologias` | 14 | `docker_kubernetes` |
| `trilhas_qualificacao` | 7 | `cloud` |

Para a demonstração ao vivo: edite um valor no Console (ex.: `salario_medio_ti_usd` de Portugal),
recarregue o app (tecla `R`) e mostre a mudança refletida sem mexer no código.

> ⚠️ Salários, custo de vida e demanda são **estimativas**, e as empresas/vagas são **fictícias** —
